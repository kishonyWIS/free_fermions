...
if __name__ == '__main__':
    ...
    for J, pulse_length in [(np.pi / 4 * 1, 1/6), (np.pi / 4 * 0.9, 1/2)]:
        ...
        plt.plot(unitary_vs_omega_close_to_vortex,angular_frequencies)
        plt.xticks([])
        plt.ylim(-np.pi*1.05, np.pi*1.05)
        plt.yticks(np.arange(-2, 3) * np.pi / 2, [r'$-\pi$', r'$-\frac{\pi}{2}$', r'$0$', r'$\frac{\pi}{2}$', r'$\pi$'])
        plt.xlim(0, np.max(np.abs(unitary_vs_omega_close_to_vortex)))
        ax.grid(axis='y')

...
results_df = pd.read_csv("KSL_results_from_cluster_larger_B.csv").query(f"kappa == {kappa} & g == {g0} & B == {B0} & errors_per_cycle_per_qubit == {errors_per_cycle_per_qubit} & initial_state == '{initial_state}' & periodic_bc == {periodic_bc}")
...
with sns.axes_style("whitegrid"):
    ...
    for trotter_steps_per_T in [40,60]:
        color = next(colors)
        marker = next(markers)
        results_df_dt = results_df.query(f"T*{trotter_steps_per_T} == Nt")
        plt.plot(results_df_dt["T"], results_df_dt["energy_density"], linestyle=None, marker=marker, color=color, label=f'Nt={trotter_steps_per_T}T')
plt.legend()
# plt.xscale('log')
plt.yscale('log')
print()

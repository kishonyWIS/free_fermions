...
with sns.axes_style("whitegrid"):
    ...
    for T in T_list:
        ...
        plt.plot(results_df_T_vs_x["num_sites_x"], results_df_T_vs_x["energy"], linestyle=None, marker=marker, color=color, label=f'T={T}')
        plt.plot(results_df_T_vs_y["num_sites_y"], results_df_T_vs_y["energy"], linestyle='--', marker=marker, color=color)
    plt.xscale('log')
    plt.yscale('log')
    plt.xlabel('Length / Circumference', fontsize=str(20), fontname='Times New Roman')  # , fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig(
        f'graphs/KSL_energy_vs_sites_x_sites_y_T.pdf')
    plt.show()

...
kx_list = np.linspace(0, np.pi, 201)
ky = 0.
...
for i_kx, kx in enumerate(kx_list):
    ...
    for i_theta, theta in enumerate(theta_list):
        for i_t, time in enumerate(times):
            u[i_kx, i_theta, i_t, :, :] = get_unitary_evolution(kx, ky, theta, time)
            phases[i_kx, i_theta, i_t, :], states[i_kx, i_theta, i_t, :, :] = eig(u[i_kx, i_theta, i_t, :, :])
            # sort the eigenvalues and eigenvectors by the phase of the eigenvalues
            idx = np.argsort(-np.angle(phases[i_kx, i_theta, i_t, :]))
            phases[i_kx, i_theta, i_t, :] = phases[i_kx, i_theta, i_t, idx]
...
def get_topological_invariant(state_vs_kx_theta):
    state_vs_kx_theta = make_state_continuous(state_vs_kx_theta)

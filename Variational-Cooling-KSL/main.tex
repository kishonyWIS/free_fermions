...
\begin{align}
H_{{\rm c}}(t)=-\sum_{\mathbf{J}}B(t)\tau_{\mathbf{J}}^{z}-\sum_{\mathbf{J}}g(t)\tau_{\mathbf{J}}^{x}.
...
Following Ref.~\cite{Kishony_2025}, we simulataneously fermionize the system and bath spins, introducing Majorana operators $\{c_{\mathbf{J}}^{x},c_{\mathbf{J}}^{y},c_{\mathbf{J}}^{z},b_{\mathbf{J}}^{x},b_{\mathbf{J}}^{y},b_{\mathbf{J}}^{z}\}$ per site. After fermionization, the Hamiltonian can be expressed in terms of the system fermions $\{c_{\mathbf{J}}^{z}\}$ and the conserved $\mathbb{Z}_2$ gauge fields $\{u_{\mathbf{I},\mathbf{J}}\}$, where $u_{\mathbf{I},\mathbf{J}} = ib_{\mathbf{I}}^{\alpha}b_{\mathbf{J}}^{\alpha}$ for bonds $\langle \mathbf{I},\mathbf{J}\rangle\in \alpha\text{-bonds}$ with $\alpha\in\{x,y,z\}$. The gauge-invariant flux $W_{i,j}$ of the $\mathbb{Z}_2$ gauge field is defined on each hexagonal plaquette corresponding to unit cell $(i,j)$ as the product of $u_{\mathbf{I},\mathbf{J}}$ on its edges. In the flux-free sector with $W_{i,j}=1$ for all plaquettes (equivalent to setting $u_{\mathbf{I},\mathbf{J}}=1$ up to a $\mathbb{Z}_2$ gauge transformation) and with uniform $\tau^z_{\mathbf{J}}=1$, the Hamiltonian becomes translationally invariant and can be Fourier transformed to momentum space.
...
In each cooling cycle, Ref.~\cite{Kishony_2025} implements a continuous-time evolution during which $g(t)$ and $B(t)$ are adiabatically varied in order to extract energy and entropy from the system to the bath through many Trotter steps. Here, we replace this adiabatic evolution with a shallow-depth variational circuit with $p$ layers, where each layer applies unitary gates corresponding to the different terms in the Hamiltonian:
